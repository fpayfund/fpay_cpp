...
using namespace sox;
namespace fapy { namespace protocol {

    bool signValidate(Pack& pk,const Byte32& public_key)
	{
		unsigned char hash[HASH256_SIZE];
		Hash256(pk.data(),pk.size(),hash);
		...
		//签名验证
        bool ret = ECKey_Verify(ecKey,hash, &(sign.u8[0]), &(sign.u8[0])+32);
		ECKEY_free(ecKey);
	...
	void genSign(const Pack& pk,const Byte32& private_key,Byte64& sign)
	{
...
free:
	...
	bool confirmation_info_t::signValidate()
	{
	    ...
	    return signValidate(pk,public_key);	
	}
